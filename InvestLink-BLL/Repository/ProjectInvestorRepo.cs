...
namespace InvestLink_BLL.Repository
{
    public class ProjectInvestorRepo : IProjectInvestor
    {
        ...
        public async Task<IEnumerable<ProjectInvestor>> GetAllAsync(int investorId)
        {
            var data = await db.ProjectInvestors.Where(p=> p.InvestorId == investorId).ToListAsync();
            return data;
        }
